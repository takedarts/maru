...
To run the Docker version of Maru using GPU, you need to have NVIDIA drivers compatible with CUDA Version 13.0 or later and NVIDIA Container Toolkit installed.
...
The Docker image intended for use with CUDA, `takedarts/maru:v8.2-cuda13.0`, is about 3 GB in size, so the download may take some time.
I recommend downloading the Docker image in advance by running the following command:
```
docker pull takedarts/maru:v8.2-cuda13.0
```
...
```
docker run -iq --rm --gpus all -v .:/workspace takedarts/maru:v8.2-cuda13.0 /opt/run.sh <model_file>
```
...
```
docker run -iq --rm --gpus all -v .:/workspace takedarts/maru:v8.2-cuda13.0 /opt/run.sh --help
```
